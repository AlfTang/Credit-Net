...
for i = 1:size(E, 1),
    ...
    for j = 1:length(F),
        ...
        if (~isempty(indx)),
            ...
            %get F(j)'s assignment
            assignment = IndexToAssignment(1:prod(F(j).card),F(j).card);

            %get all indices with respect to the variable which is inconsistent with evidence
            indices = find(assignment(:,indx) ~= x);

            %get all such assignments
            assign_set = IndexToAssignment(indices,F(j).card);

            %set all value of such assignment to zero
